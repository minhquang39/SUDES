<template>
  <div>
    ...
    <div v-else class="bg-primaryBg px-[14px] lg:px-11 xl:px-30 py-6">
      <div v-if="isLoading">
        <CategoryDetailSkeleton />
      </div>
      <div v-else>
        ...
        <p class="text-center text-[#333] text-[16px] mt-4">
          ...
          khách hàng những sản phẩm chất lượng nhất - tốt nhất - tinh hoa nhất với đội ngũ chuyên
          gia nghiên cứu dinh dưỡng hàng đầu Việt Nam và luôn đầu tư dây chuyền sản xuất công nghệ
          cao, hiện đại, quy mô sản xuất lớn.
        </p>
...
export default {
  ...
  methods: {
    ...
    async loadCategoryData(categorySlug) {
      ...
      if (this.initialLoading) {
        console.log('Initial loading is true')
      } else {
        this.isLoading = true
      }

      console.log(
        'Loading data for slug:',
        categorySlug,
        'isLoading:',
        this.isLoading,
        'initialLoading:',
      ...
      try {
        ...
        const response = await apiClient.get(`/admin/category/slug/${categorySlug}`)
  ...
  created() {
    console.log(
      'Component created, initialLoading:',
      this.initialLoading,
      'isLoading:',
      this.isLoading,
    )
    this.loadCategoryData(this.$route.params.slug)
  },
  watch: {
    '$route.params.slug': function (newSlug) {
      console.log('Slug changed to:', newSlug)
      this.loadCategoryData(newSlug)
    },
  },
}
</script>
